---
title: "Tools: Typed Agent Actions with defineTool for eve"
sidebarTitle: "Tools"
...
The framework defaults are importable from `eve/tools/defaults` — `bash`, `readFile`, `writeFile`, `glob`, `grep`, `webFetch`, `webSearch`, `todo`, `loadSkill`. Author a file at the same slug to replace the built-in:

```ts agent/tools/write_file.ts
import { defineTool } from "eve/tools";
